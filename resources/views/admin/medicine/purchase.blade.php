...
<main id="main" class="main leaves-bg">
    <div class="pagetitle d-flex align-items-center justify-content-between">
        <h3 class="text-primary">Medicine Purchase</h3>
    ...
    <section class="section">
        <form id="formData" method="POST" action="{{ route('medicinePurchase.store') }}">
            @csrf
                    ...
                    <div class="row mb-3 bg-white py-3 rounded-3 justify-content-between">
                        <div class="col-md-4">
                            <div class="d-flex align-items-center">
                                <label for="invoice_number" class="fw-semibold w-75">Invoice No</label>
                                <input type="text" class="form-control @error('invoice_number') is-invalid @enderror" name="invoice_number" id="invoice_number"
                                    oninput="fetchData()" value="{{ Request::get('invoiceNo') ? Request::get('invoiceNo') : old('invoice_number')}}">
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="d-flex gap-md-3 align-items-center">
                                <label for="invoice_date" class="fw-semibold">Dated</label>
                                <input type="date" class="form-control @error('invoice_date') is-invalid @enderror" name="invoice_date" id="invoice_date"
                                    placeholder="Dated" value="{{ now()->format('Y-m-d') }}" max="{{now()->format('Y-m-d')}}">
                            </div>
                        </div>
                        <div class="col-md-4 pe-md-0">
                            <div class="d-flex gap-md-3 align-items-center">
                                <label for="supplier" class="fw-semibold w-auto @error('supplier') is-invalid @enderror">Purchase Party</label>
                                <select class="form-select w-50" aria-label="Default select example" name="supplier"
                                    id="supplier">
                    ...
                    <div class="row mb-3 bg-white py-3 rounded-3 justify-content-between">
                        <div class="col-md-4 pe-md-3">
                            <div class="d-flex gap-2 align-items-center">
                                ...
                                <select class="form-select @error('payment_method') is-invalid @enderror" name="payment_method" id="payment_method"
                                    aria-label="Default select example" onchange="handleBank()">
                                    <option value="">Select Payment Method</option>
                                    <option @if(old('payment_method')=="Cash") selected @endif value="Cash">Cash</option>
                                    <option @if(old('payment_method')=="Bank") selected @endif value="Bank">Bank Transfer</option>
                                </select>
                            </div>
                        </div>
                        <div class="col-md-4 pe-md-3">
                        ...
                        <div class="col-md-4 pe-md-3">
                            <div class="d-flex gap-2 align-items-center">
                                ...
                                <input type="text" class="form-control" id="narration" name="narration" value={{old('narration')}}>
                            </div>
                        </div>
                    </div>
                    <div class="row mb-3 bg-white py-3 rounded-3 justify-content-between row-gap-3">
                        ...
                        <div class="col-md-2">
                            <div>
                                ...
                                <input type="number" step="any" class="form-control" id="total_amount" name="total_amount"
                                    oninput="calculateNet()" readonly>
                            </div>
                        </div>
                        <div class="col-md-2">
                            <div>
                                <label for="tax" class="fw-semibold">Tax</label>
                                <input type="number" step="any" class="form-control" id="tax" name="tax" oninput="calculateNet()">
                            </div>
                        </div>
                        <div class="col-md-2">
                            <div>
                                <div>
                                    ...
                                    <input type="number" step="any" class="form-control" id="net_amount" name="net_amount"
                                        readonly>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-12">
                            <div class="d-flex justify-content-end">
                                <button class="btn btn-primary mt-0 fs-6" type="button"
                                    onclick="storeData()">Add Medicine</button>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="texes">
                        <div class="taxes-inner">
                            <div class="d-flex align-items-center gap-1">
                                ...
                                <input type="number" step="any" name="cgst" id="cgst" class="form-control" value="{{old('cgst')}}" oninput="calculateGST()">
                            </div>
                        </div>
                        <div class="taxes-inner">
                            <div class="d-flex align-items-center gap-1">
                                <label for="sgst" class="fw-semibold">SGST</label>
                                <input type="number" step="any" name="sgst" id="sgst" class="form-control" value="{{old('sgst')}}" oninput="calculateGST()">
                            </div>
                        </div>
                        <div class="taxes-inner">
                            <div class="d-flex align-items-center gap-1">
                                <label for="gst" class="fw-semibold">GST</label>
                                <input type="number" step="any" name="gst" id="gst" class="form-control" value="{{old('gst')}}"
                        ...
                        <div class="taxes-inner">
                            <div class="d-flex align-items-center gap-1">
                                ...
                                <input type="number" step="any" name="freight" id="freight" class="form-control" value="{{old('freight')}}"
                                    oninput="calculateInvoiceNet()">
                            </div>
                        </div>
                        <div class="taxes-inner">
                            <div class="d-flex align-items-center gap-1">
                                <label for="labour" class="fw-semibold">Labour</label>
                                <input type="number" step="any" name="labour" id="labour" class="form-control" value="{{old('labour')}}"
                                    oninput="calculateInvoiceNet()">
                            </div>
                        </div>
                        <div class="taxes-inner">
                            <div class="d-flex align-items-center gap-1">
                                <label for="invoice_total" class="fw-semibold @error('invoice_total') is-invalid @enderror">Total</label>
                                <input type="number" step="any" name="invoice_total" id="invoice_total" class="form-control" value="{{old('invoice_total')}}"
                                    oninput="calculateInvoiceNet()" readonly>
                            </div>
                        </div>
                        <div class="taxes-inner">
                            <div class="d-flex align-items-center gap-1">
                                <label for="invoice_net" class="fw-semibold @error('invoice_net') is-invalid @enderror">Net</label>
                                <input type="number" step="any" name="invoice_net" id="invoice_net" class="form-control" readonly>
                            </div>
                        </div>
                    </div>
            ...
            <div class="modal-body">
                <form id="modifiedData">
                    ...
                    <div class="row row-gap-3 justify-content-center">
                        ...
                        <div class="col-md-6">
                            <div class="form-floating">
                                <input type="number" step="any" class="form-control" id="modifiedQuantity" name="modifiedQuantity"
                                    placeholder="Quantity" oninput="calculateModifiedTotal()">
                        ...
                        <div class="col-md-6">
                            <div class="col-md-12">
                                <div class="form-floating">
                                    <input type="number" step="any" class="form-control" name="modifiedRate" id="modifiedRate"
                                        placeholder="Rate" oninput="calculateModifiedTotal()">
                        ...
                        <div class="col-md-6">
                            <div class="col-md-12">
                                <div class="form-floating">
                                    <input type="number" step="any" class="form-control" name="modifiedTax" id="modifiedTax"
                                        placeholder="Tax" oninput="calculateModifiedNet()">
...
<script>
    ...
    function storeData() {
        ...
        $.ajax({
            type: 'POST',
            url: '{{ route('medicinePurchase.storeData') }}',
            data: formData,
    ...
    function fetchData() {
        ...
        $.ajax({
            type: 'GET',
            url: '{{ route('medicinePurchase.fetchData') }}',
            data: {
    ...
    function displayData(data) {
        // Display Invoice Details
            if (data.invoiceDetail && data.invoiceDetail.length > 0) {
                ...
                $('#supplier').val(invoiceDetail.supplier);
                $('#payment_method').val(invoiceDetail.payment_method);
                $('#bank').val(invoiceDetail.bank);
                $('#narration').val(invoiceDetail.narration);
                $('#cgst').val(invoiceDetail.cgst);
                $('#sgst').val(invoiceDetail.sgst);
                $('#gst').val(invoiceDetail.gst);
                $('#freight').val(invoiceDetail.freight);
                $('#labour').val(invoiceDetail.labour);
                $('#invoice_total').val(invoiceDetail.invoice_total);
                $('#invoice_net').val(invoiceDetail.invoice_net);
            }
    ...
    function deleteData() {
        ...
        $.ajax({
            type: 'DELETE',
            url: '{{ route('medicinePurchase.deleteData') }}',
            data: deletedData,
            success: function (response) {
                fetchData();
    ...
    $('#dataTable').on('click', '.modifyPurchase', function () {
        ...
        $.ajax({
            url: "{{ route('medicinePurchase.editData', '') }}/" +
                modifyId,
            type: "GET",
            data: "",
            dataType: "",
    ...
    function updateData() {
        ...
        $.ajax({
            type: 'PUT',
            url: '{{ route('medicinePurchase.updateData') }}',
            data: modifiedData,
            success: function (response) {
                fetchData();
