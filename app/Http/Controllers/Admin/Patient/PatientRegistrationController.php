...
class PatientRegistrationController extends Controller
{
    public function index() {
        ...
        $fileNo = PatientPersonalDetail::latest('id')->value('id');
        if ($fileNo) {
            $fileNo = $fileNo + 1;
        } else {
            $fileNo = 1;
        ...
        return view('admin.patient.register', compact('auth_token', 'countries', 'questions', 'fileNo'));
    }
    ...
    public function store(Request $request) {
        ...
        if ($validator->passes()) {
            ...
            $patientPersonalDetail->status_id = 1;

            if ($request->file('photo')) {
                $photo = $request->file('photo');
                $photo_name = time() .'-gc.'. $photo->getClientOriginalExtension();
                $photo->storeAs('public/uploads/PatientPhotos', $photo_name);
            ...
            return redirect() -> route('admin.login') -> with('success', 'Patient Details Inserted Successfully');
        } else {
            return redirect() -> route('registerPatient.create') -> withErrors($validator) -> withInput($request->all()) -> with('error', 'Please Check All Inputs');
        }
